...
# Direct-submodule imports only here: this file loads while
# nanobot.persistence.__init__ is still executing, so attribute access on the
# half-built package would be a circular-import gamble.
from nanobot.persistence.restore_gate import is_held as _restore_pending
...
DEFAULT_TREE_EXCLUDES: tuple[str, ...] = (
    ...
    "**/*.tmp",
    "**/.DS_Store",
    # Degraded-start gate marker (see nanobot.persistence.restore_gate). It is
    # per-container runtime state, never user data, and must not travel inside
    # the archive to another container.
    ".restore-pending",
)
...
class TreeArchiveMirror:
    ...
    async def snapshot(self, *, force: bool = False) -> int:
        ...
        # Degraded start (restore-pending marker): this container never verified
        # its local tree against the mirror, so pushing could overwrite healthy
        # remote state with a template-only tree. Skip every push; the gateway's
        # background restorer clears the marker once the mirror is fully pulled,
        # and normal cadence resumes on the next cycle.
        if not force and _restore_pending(self._root):
            logger.warning(
                "Supabase tree mirror: snapshot skipped — restore still pending "
                "(degraded start); mirror stays untouched"
            )
            return 0
