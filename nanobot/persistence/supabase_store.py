...
# Direct-submodule import: this file loads while nanobot.persistence.__init__
# is still executing, so attribute access on the half-built package would be a
# circular-import gamble.
from nanobot.persistence.restore_gate import default_data_dir
...
class WorkspaceStateMirror:
    ...
    async def snapshot(self, *, force: bool = False) -> list[str]:
        ...
        # its local files against Supabase, so pushing could overwrite healthy
        # rows (cron jobs, MCP tokens) with partial state. Skip every push; the
        # gateway's background restorer clears the marker once the full mirror
        # has been pulled, and normal cadence resumes on the next cycle.
        if not force and _restore_pending(
            self._data_path if self._data_path is not None else default_data_dir()
        ):
            logger.warning(
                "Supabase mirror: snapshot skipped — restore still pending "
