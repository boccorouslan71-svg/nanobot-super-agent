...
0 means "start", non-zero means "refuse to start", and getting that backwards
either blocks a healthy boot or lets a fresh container overwrite the mirror.
"""
...
def test_restore_failure_refuses_startup_after_retries(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    _creds(monkeypatch, tmp_path)
    attempts = 0
