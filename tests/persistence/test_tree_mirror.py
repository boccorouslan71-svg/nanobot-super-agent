...
@pytest.mark.asyncio
async def test_snapshot_is_frozen_while_the_restore_marker_holds(tmp_path: Path) -> None:
    """Degraded start: a container that never verified its tree must not push."""
    ...
    restore_gate.place(root, "test: degraded start")
    assert await mirror.snapshot() == 0
    assert store.pushes == 0

    # The marker itself is per-container runtime state, never user data.
    names = {p.relative_to(root).as_posix() for p in mirror.iter_files()}
    assert ".restore-pending" not in names

    # Clearing the marker (a full restore succeeded) unfreezes the cadence.
    restore_gate.clear(root)
    assert await mirror.snapshot() > 0
    assert store.pushes == 1


@pytest.mark.asyncio
async def test_force_snapshot_still_pushes_through_a_held_marker(
    tmp_path: Path,
) -> None:
    """force=True is an explicit human decision and bypasses the gate on purpose."""
    from nanobot.persistence import restore_gate

    store = FakeStore()
    mirror = _mirror(_tree(tmp_path), store)

    restore_gate.place(tmp_path, "test: degraded start")
    assert await mirror.snapshot(force=True) > 0
    assert store.pushes == 1
